...
public class ModRMSIB {
	...
	private void Make(Reg64 rdisp, int disp, Reg r) {
		...
		if (rdisp == Reg64.RSP) {
			int doubler = ((0 << 6) | (getIdx(rdisp) << 3) | getIdx(rdisp));
			_b.write(doubler);
		}
	...
	private void Make( Reg64 ridx, int mult, int disp, Reg r ) {
		...
		_b.write(regByte);
		int multiplyByte = ((ss << 6) | (getIdx(ridx) << 3) | getIdx(Reg64.RBP));
		_b.write(multiplyByte);
		x64.writeInt(_b, disp);
	}
	...
	private void Make( Reg64 rdisp, Reg64 ridx, int mult, int disp, Reg r ) {
		...
		int regByte = (mod << 6) | (getIdx(r) << 3) | getIdx(Reg64.RSP);
		_b.write(regByte);
		int multiplyByte = ((ss << 6) | (getIdx(ridx) << 3) | getIdx(rdisp));
		_b.write(multiplyByte);
		x64.writeInt(_b, disp);
	}
